...
class SkipGram:
    def __init__(self):
        self.data_index = 0
        self.trainpath = './data/regionname_clean.txt'
        self.modelpath = './model/region_wordvec.bin'
        self.min_count = 1#最低词频，保留模型中的词表
        self.batch_size = 256 # 每次迭代训练选取的样本数目
        self.embedding_size = 100  # 生成词向量的维度
        self.window_size = 1  # 考虑前后几个词，窗口大小, skipgram中的中心词-上下文pairs数目就是windowsize *2
        self.num_sampled = 50  # 负样本采样.
    ...
    def train_wordvec(self, vocabulary_size, batch_size, embedding_size, window_size, num_sampled, num_steps, data, count):
        ...
        with graph.as_default():
            ...
            with tf.device('/cpu:0'):
                ...
                embed = tf.nn.embedding_lookup(embeddings, train_inputs)  #eg. tf.nn.embedding_lookup(embeddings, [1,3,5])选取索引值为1，3，5的张量
        ...
        with tf.Session(graph = graph) as session:
            ...
            print(graph)
        #saver.save(session, './model_results/skipgram_model.ckpt', global_step=step)
    ...
    def train(self):
        ...
        """
        final_embeddings = self.train_wordvec(vocabulary_size, self.batch_size, self.embedding_size, self.window_size, self.num_sampled, self.num_steps, data, count)
