...
'''
def Attention(Q, K, V, head_number, size_per_head, Q_len=None, V_len=None):
    #对Q、K、V分别作线性映射
    Q = Dense(Q, head_number * size_per_head, False)
    Q = tf.reshape(Q, (-1, tf.shape(Q)[1], head_number, size_per_head))
    Q = tf.transpose(Q, [0, 2, 1, 3])
    K = Dense(K, nb_head * size_per_head, False)
    K = tf.reshape(K, (-1, tf.shape(K)[1], head_number, size_per_head))
    K = tf.transpose(K, [0, 2, 1, 3])
    V = Dense(V, nb_head * size_per_head, False)
    V = tf.reshape(V, (-1, tf.shape(V)[1], head_number, size_per_head))
    ...
    O = tf.reshape(O, (-1, tf.shape(O)[1], head_number * size_per_head))
    O = Mask(O, Q_len, 'mul')
    return O